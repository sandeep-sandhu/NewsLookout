...
"""
 File name: test_mod_en_in_livemint.py
 Application: The NewsLookout Web Scraping Application
 Date: 2021-06-01
 Purpose: Test for the mod_en_in_livemint plugin for the web scraping and news text processing application
...
def test_plugin_subclass():
    ...
    from newslookout.plugins.mod_en_in_livemint import mod_en_in_livemint
    from newslookout import data_structs
    from newslookout import session_hist

    pluginClassInst = mod_en_in_livemint()
    print(f'Instantiated plugins name: {pluginClassInst.pluginName}')
    assert type(pluginClassInst).__name__ == "mod_en_in_livemint", \
        "mod_en_in_livemint Plugin was not initialising correctly"
    pluginClassInst.config(app_inst.app_config)
    print(f'Base data directory configured as {pluginClassInst.baseDirName}')
    assert len(pluginClassInst.baseDirName) > 0, "mod_en_in_livemint Plugin not configured: baseDirName!"
    assert pluginClassInst.configReader is not None, "mod_en_in_livemint Plugin not configured: configReader!"
    assert len(pluginClassInst.urlMatchPatterns) > 0, "mod_en_in_livemint Plugin not configured: urlMatchPatterns!"
    # assert len(pluginClassInst.authorMatchPatterns) > 0, "mod_en_in_livemint not configured: authorMatchPatterns!"
    assert len(pluginClassInst.dateMatchPatterns) > 0, "mod_en_in_livemint Plugin not configured: dateMatchPatterns!"
    print(f'mod_en_in_livemint plugin {pluginClassInst.getStatusString()}')
    assert pluginClassInst.getStatusString() == 'State = STATE_GET_URL_LIST', \
        "mod_en_in_livemint Plugin status not set correctly!"
    pluginClassInst.initNetworkHelper()
    assert type(pluginClassInst.networkHelper) == newslookout.network.NetworkFetcher, "mod_en_in_livemint network fetcher not init!"
    pluginClassInst.setURLQueue(queue.Queue())
    assert type(pluginClassInst.urlQueue) == queue.Queue, "mod_en_in_livemint queue not set!"
...
def test_fetchDataFromURL():
    ...
    uRLtoFetch = "https://www.livemint.com/money/personal-finance/term-insurance-policy-premium-likely-to-get-costlier-11613646112891.html"
    ...
    assert resultVal.publishDate == datetime.strptime(
        '2021-02-18', '%Y-%m-%d'), 'fetchDataFromURL() not parsing published date correctly.'
    assert resultVal.articleID == '11613646112891', 'fetchDataFromURL() not identifying unique ID correctly.'
    assert resultVal.textSize > 2000, 'fetchDataFromURL() not parsing text body correctly.'
    assert resultVal.savedDataFileName == os.path.join(app_inst.app_config.data_dir, '2021-02-18', 'mod_en_in_livemint_11613646112891'), \
        'fetchDataFromURL() not saving parsed data correctly.'
    assert len(resultVal.additionalLinks) == 4, 'fetchDataFromURL() not extracting additional links correctly.'
...
def test_extractUniqueIDFromURL():
    ...
    assert uniqueID == '11613646112891', "extractUniqueIDFromURL() is not correctly identifying article unique ID"
