...
def testPluginSubClass():
    ...
    assert type(pluginClassInst).__name__ == "mod_in_bse", \
        "mod_in_bse Plugin was not initialising correctly"
    pluginClassInst.config(app_inst.app_config)
    print(f'Base data directory configured as {pluginClassInst.baseDirName}')
    assert len(pluginClassInst.baseDirName) > 0, "mod_in_bse Plugin not configured: baseDirName!"
    assert pluginClassInst.configReader is not None, "mod_in_bse Plugin not configured: configReader!"
    assert len(pluginClassInst.urlMatchPatterns) > 0, "mod_in_bse Plugin not configured: urlMatchPatterns!"
    # assert len(pluginClassInst.dateMatchPatterns) > 0, "mod_in_bse Plugin not configured: dateMatchPatterns!"
    print(f'mod_in_bse plugin {pluginClassInst.getStatusString()}')
    print(f'mod_in_bse pluginState = {pluginClassInst.pluginState}')
    assert pluginClassInst.getStatusString() == 'State = STATE_GET_URL_LIST', \
        "mod_in_bse Plugin status not set correctly!"
    pluginClassInst.initNetworkHelper()
    assert type(pluginClassInst.networkHelper) == network.NetworkFetcher, "mod_in_bse network fetcher not init!"
    pluginClassInst.setURLQueue(queue.Queue())
    assert type(pluginClassInst.urlQueue) == queue.Queue, "mod_in_bse queue not set!"
...
def test_extractUniqueIDFromURL():
    global pluginClassInst
    print(f'Instantiated plugins name: {pluginClassInst.pluginName}')
    uRLtoFetch = "https://www.bseindia.com/download/BhavCopy/Equity/EQ_ISINCODE_140520.zip"
    (publishDate, uniqueID) = pluginClassInst.extractUniqueIDFromURL(uRLtoFetch)
    assert uniqueID == '140520', "extractUniqueIDFromURL() is not correctly identifying article unique ID"
