...
"""
 File name: test_mod_en_in_moneycontrol.py
 Application: The NewsLookout Web Scraping Application
 Date: 2021-06-01
 Purpose: Test for the mod_en_in_moneycontrol plugin for the web scraping and news text processing application
...
def testPluginSubClass():
    ...
    assert type(pluginClassInst).__name__ == "mod_en_in_moneycontrol", \
        "mod_en_in_moneycontrol Plugin was not initialising correctly"
    pluginClassInst.config(app_inst.app_config)
    print(f'Base data directory configured as {pluginClassInst.baseDirName}')
    assert len(pluginClassInst.baseDirName) > 0, "mod_en_in_moneycontrol Plugin not configured: baseDirName!"
    assert pluginClassInst.configReader is not None, "mod_en_in_moneycontrol Plugin not configured: configReader!"
    assert len(pluginClassInst.urlMatchPatterns) > 0, "mod_en_in_moneycontrol Plugin not configured: urlMatchPatterns!"
    # assert len(pluginClassInst.authorMatchPatterns) > 0, "mod_en_in_moneycontrol not configured: authorMatchPatterns!"
    assert len(pluginClassInst.dateMatchPatterns) > 0, "mod_en_in_moneycontrol Plugin not configured: dateMatchPatterns!"
    print(f'mod_en_in_moneycontrol plugin {pluginClassInst.getStatusString()}')
    assert pluginClassInst.getStatusString() == 'State = STATE_GET_URL_LIST', \
        "mod_en_in_moneycontrol Plugin status not set correctly!"
    pluginClassInst.initNetworkHelper()
    assert type(pluginClassInst.networkHelper) == network.NetworkFetcher, "mod_en_in_moneycontrol network fetcher not init!"
    pluginClassInst.setURLQueue(queue.Queue())
    assert type(pluginClassInst.urlQueue) == queue.Queue, "mod_en_in_moneycontrol queue not set!"
...
def test_fetchDataFromURL():
    ...
    uRLtoFetch = "https://www.moneycontrol.com/news/business/ipo/zomato-ipo-improving-economics-growth-plans-and-strong-investor-appetite-7173681.html"
    ...
    assert resultVal.savedDataFileName == os.path.join('./data', '2021-02-18', 'mod_en_in_moneycontrol_7173681'), \
    ...
    nonURL1 = 'https://www.moneycontrol.com/glossary/'
    nonURL2 = 'https://www.moneycontrol.com/msite/decoding-the-world-of-etf/?utmsrc=1234'
    resultList = pluginClassInst.filterNonContentURLs([nonURL1, nonURL2, uRLtoFetch])
    print(f'Result after filtering out non-content URLS: {resultList}')
    assert nonURL1 not in resultList, "filterNonContentURLs() is not removing non-article URLs from fetch list."
