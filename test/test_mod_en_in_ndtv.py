...
"""
 File name: test_mod_en_in_ndtv.py
 Application: The NewsLookout Web Scraping Application
 Date: 2021-06-01
 Purpose: Test for the mod_en_in_ndtv plugin for the web scraping and news text processing application
...
def test_plugin_subclass():
    ...
    from newslookout.plugins.mod_en_in_ndtv import mod_en_in_ndtv
    from newslookout import data_structs
    from newslookout import session_hist

    pluginClassInst = mod_en_in_ndtv()
    print(f'Instantiated plugins name: {pluginClassInst.pluginName}')
    assert type(pluginClassInst).__name__ == "mod_en_in_ndtv", \
        "mod_en_in_ndtv Plugin was not initialising correctly"
    pluginClassInst.config(app_inst.app_config)
    print(f'Base data directory configured as {pluginClassInst.baseDirName}')
    assert len(pluginClassInst.baseDirName) > 0, "mod_en_in_ndtv Plugin not configured: baseDirName!"
    assert pluginClassInst.configReader is not None, "mod_en_in_ndtv Plugin not configured: configReader!"
    assert len(pluginClassInst.urlMatchPatterns) > 0, "mod_en_in_ndtv Plugin not configured: urlMatchPatterns!"
    # assert len(pluginClassInst.authorMatchPatterns) > 0, "mod_en_in_ndtv not configured: authorMatchPatterns!"
    assert len(pluginClassInst.dateMatchPatterns) > 0, "mod_en_in_ndtv Plugin not configured: dateMatchPatterns!"
    print(f'mod_en_in_ndtv plugin {pluginClassInst.getStatusString()}')
    assert pluginClassInst.getStatusString() == 'State = STATE_GET_URL_LIST', \
        "mod_en_in_ndtv Plugin status not set correctly!"
    pluginClassInst.initNetworkHelper()
    assert type(pluginClassInst.networkHelper) == network.NetworkFetcher, "mod_en_in_ndtv network fetcher not init!"
    pluginClassInst.setURLQueue(queue.Queue())
    assert type(pluginClassInst.urlQueue) == queue.Queue, "mod_en_in_ndtv queue not set!"
...
def test_fetchDataFromURL():
    ...
    uRLtoFetch = "https://www.ndtv.com/business/dollar-rises-on-us-recovery-hopes-bitcoin-hits-near-record-52-640-2373245"
    ...
    assert resultVal.textSize > 3000, 'fetchDataFromURL() not parsing text body correctly.'
    assert resultVal.savedDataFileName == os.path.join(app_inst.app_config.data_dir, '2021-02-18', 'mod_en_in_ndtv_2373245'), \
...
def test_extractUniqueIDFromURL():
    ...
    assert uniqueID == '2373245', "extractUniqueIDFromURL() is not correctly identifying article unique ID"
