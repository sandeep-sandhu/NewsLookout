...
"""
 File name: test_mod_en_in_business_std.py
 Application: The NewsLookout Web Scraping Application
 Date: 2021-06-01
 Purpose: Test for the mod_en_in_business_std plugin for the web scraping and news text processing application
...
def testPluginSubClass():
    ...
    assert type(pluginClassInst).__name__ == "mod_en_in_business_std", \
        "mod_en_in_business_std Plugin was not initialising correctly"
    pluginClassInst.config(app_inst.app_config)
    print(f'Base data directory configured as {pluginClassInst.baseDirName}')
    assert len(pluginClassInst.baseDirName) > 0, "mod_en_in_business_std Plugin not configured: baseDirName!"
    assert pluginClassInst.configReader is not None, "mod_en_in_business_std Plugin not configured: configReader!"
    assert len(pluginClassInst.urlMatchPatterns) > 0, "mod_en_in_business_std Plugin not configured: urlMatchPatterns!"
    # assert len(pluginClassInst.authorMatchPatterns) > 0, "mod_en_in_business_std not configured: authorMatchPatterns!"
    assert len(pluginClassInst.dateMatchPatterns) > 0, "mod_en_in_business_std Plugin not configured: dateMatchPatterns!"
    print(f'mod_en_in_business_std plugin {pluginClassInst.getStatusString()}')
...
def test_fetchDataFromURL():
    ...
    uRLtoFetch = "https://www.business-standard.com/article/pti-stories/mukesh-ambani-urges-pm-to-take-steps-against-data-colonisation-119011800410_1.html"
    ...
    assert resultVal.savedDataFileName == './data\\2019-01-18\\mod_en_in_business_std_119011800410', \
    ...
    with open(resultVal.savedDataFileName + ".json", 'rt') as fp:
        doc_obj = json.load(fp)
