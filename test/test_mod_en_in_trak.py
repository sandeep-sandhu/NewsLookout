...
"""
 File name: test_mod_en_in_trak.py
 Application: The NewsLookout Web Scraping Application
 Date: 2021-06-01
 Purpose: Test for the mod_en_in_trak plugin for the web scraping and news text processing application
...
def test_plugin_subclass():
    ...
    from newslookout.plugins.mod_en_in_trak import mod_en_in_trak
    from newslookout import data_structs
    from newslookout import session_hist

    pluginClassInst = mod_en_in_trak()
    print(f'Instantiated plugins name: {pluginClassInst.pluginName}')
    assert type(pluginClassInst).__name__ == "mod_en_in_trak", \
        "mod_en_in_trak Plugin was not initialising correctly"
    pluginClassInst.config(app_inst.app_config)
    print(f'Base data directory configured as {pluginClassInst.baseDirName}')
    assert len(pluginClassInst.baseDirName) > 0, "mod_en_in_trak Plugin not configured: baseDirName!"
    assert pluginClassInst.configReader is not None, "mod_en_in_trak Plugin not configured: configReader!"
    assert len(pluginClassInst.urlMatchPatterns) > 0, "mod_en_in_trak Plugin not configured: urlMatchPatterns!"
    assert len(pluginClassInst.authorMatchPatterns) > 0, "mod_en_in_trak not configured: authorMatchPatterns!"
    assert len(pluginClassInst.dateMatchPatterns) > 0, "mod_en_in_trak Plugin not configured: dateMatchPatterns!"
    print(f'mod_en_in_trak plugin {pluginClassInst.getStatusString()}')
    assert pluginClassInst.getStatusString() == 'State = STATE_GET_URL_LIST', \
        "mod_en_in_trak Plugin status not set correctly!"
    pluginClassInst.initNetworkHelper()
    assert type(pluginClassInst.networkHelper) == network.NetworkFetcher, "mod_en_in_trak network fetcher not init!"
    pluginClassInst.setURLQueue(queue.Queue())
    assert type(pluginClassInst.urlQueue) == queue.Queue, "mod_en_in_trak queue not set!"
...
def test_fetchDataFromURL():
    ...
    uRLtoFetch = "https://trak.in/tags/business/2021/02/18/oneplus-watch-launch-soon-spotted-on-gpto-india-launch-when-oneplus-watch-price-availability/"
    ...
    assert resultVal.articleID == '119735', 'fetchDataFromURL() not identifying unique ID correctly.'
    assert resultVal.textSize > 4000, 'fetchDataFromURL() not parsing text body correctly.'
    assert resultVal.savedDataFileName == os.path.join(app_inst.app_config.data_dir, '2021-02-18', 'mod_en_in_trak_119735'), \
        'fetchDataFromURL() not saving parsed data correctly.'
    assert len(resultVal.additionalLinks) == 25, 'fetchDataFromURL() not extracting additional links correctly.'
