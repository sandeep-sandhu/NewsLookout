...
# Date: 2021-06-23                                                                                        #
# Purpose: Queue Manager with Coordinated Worker Pairs                                                    #
...
from newslookout.data_structs import PluginTypes, QueueStatus
from newslookout.session_hist import SessionHistory
from newslookout.worker import WorkerPair, DataProcessor, StatusAPIServer
...
class QueueManager:
    ...
    def loadPlugins(app_dir, plugins_dir, contrib_dir, enabled_names):
        ...
        # Use the fully-qualified package name so Python can locate it as a package.
        # importlib.resources.contents() is deprecated since Python 3.11; use files().iterdir().
        plugin_package = "newslookout.plugins"
        try:
            plugin_files = [
                resource.name
                for resource in importlib.resources.files(plugin_package).iterdir()
            ]
        except Exception as e:
            logger.error(f"Could not list plugin package '{plugin_package}': {e}")
            plugin_files = []
        ...
        for plugin_file in plugin_files:
            ...
            if mod_name in enabled_names:
                try:
                    class_obj = getattr(
                        importlib.import_module(f"{plugin_package}.{mod_name}"),
                        mod_name
                    )
