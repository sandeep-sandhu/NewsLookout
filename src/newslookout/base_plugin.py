...
from newslookout.network import NetworkFetcher
from newslookout.data_structs import PluginTypes, ScrapeError, ExecutionResult, PluginStatus
from newslookout.news_event import NewsEvent
from newslookout.scraper_utils import normalizeURL, extractLinks, calculateCRC32, getPreviousDaysDate, getNextDaysDate
from newslookout.scraper_utils import is_valid_url
from newslookout.scraper_utils import retainValidArticles, removeInValidArticles
from newslookout.scraper_utils import sameURLWithoutQueryParams
from newslookout.session_hist import SessionHistory
from newslookout.archive_writer import get_archive_writer
...
class BasePlugin:
    ...
    def config(self, app_config: ConfigManager):
        ...
        if not isinstance(self.app_config, ConfigManager):
            raise TypeError(f"{self.pluginName}: Expected ConfigManager, got {type(self.app_config).__name__}")
        try:
    ...
    def writeFiles(self, article: NewsEvent, fileNameWithOutExt: str, htmlContent, saveHTMLFile: bool = False):
        ...
        if self.use_archive_storage and self.archive_writer is not None:
            try:
        ...
        # Legacy / test path: write JSON directly
        json_file = fileNameWithOutExt + '.json'
        os.makedirs(os.path.dirname(json_file), exist_ok=True)
        with open(json_file, 'wt', encoding='utf-8') as fp:
            fp.write(jsonContent)
        if saveHTMLFile and htmlContent:
            import bz2
            html_file = fileNameWithOutExt + '.html.bz2'
            with bz2.open(html_file, 'wb') as fp:
            ...
            if uRLtoFetch not in self.nonContentURLs:
                ...
                if htmlContent is not None and len(htmlContent) > self.minArticleLengthInChars:
                    ...
                    if validData.getTextSize() > self.minArticleLengthInChars:
                        # Guard: discard article if ID cannot be determined
                        if not validData.getArticleID() or str(validData.getArticleID()) in ('None', ''):
                            logger.warning("%s: Discarding article with no valid ID for URL: %s",
                                           self.pluginName, uRLtoFetch)
                        else:
                            ...
                            self.writeFiles(validData, savefileNameWithOutExt, htmlContent.encode('utf-8'))
    ...
    def parseFetchedData(self, uRLtoFetch: str, newpArticleObj, WorkerID: int) -> NewsEvent:
        ...
        def _is_future_date(dt):
            ...
            try:
                if dt.tzinfo is not None:
                    # strip tzinfo for comparison – we only need the calendar date
        ...
        if (newpArticleObj.publish_date is None or newpArticleObj.publish_date == '' or
                (not newpArticleObj.publish_date) or (
                        isinstance(newpArticleObj.publish_date, datetime) and
                        _is_future_date(newpArticleObj.publish_date)
                )
        ):
        ...
        try:
            ...
            authors_list = newpArticleObj.authors if isinstance(newpArticleObj.authors, list) else []
            if (len(authors_list) < 1 or
                    (len(authors_list) > 0 and authors_list[0].find('<') >= 0)):
        ...
        # If unique ID could not be determined, fall back to CRC32 of the URL
        if not articleUniqueID:
            try:
                articleUniqueID = calculateCRC32(uRLtoFetch)
                logger.warning("%s: Using CRC32 fallback article ID '%s' for URL: %s",
                               self.pluginName, articleUniqueID, uRLtoFetch)
            except Exception as crc_err:
                logger.error("%s: Cannot compute CRC32 fallback for URL %s: %s",
                             self.pluginName, uRLtoFetch, crc_err)
