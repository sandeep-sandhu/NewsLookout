...
def fixSentenceGaps(input_text: str) -> str:
    ...
    try:
        # handles word at start-of-string and single-letter words
        cleaned_text = re.sub(r'(\b[a-zA-Z]{2,})(\.)([A-Z][a-zA-Z])',
...
def cutStrBetweenTags(sourceStr, startTagStr, endTagStr) -> str:
    resultStr = ""
    try:
        raw_pos = sourceStr.find(startTagStr)
        if raw_pos > -1:                               # correct "not found" guard
            start_pos = raw_pos + len(startTagStr)
