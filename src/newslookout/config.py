...
class ConfigManager:
    ...
    def set_default_values(self):
        ...
        Chooses platform-appropriate directories for Windows, Linux, and macOS.
        ...
        # ------------------------------------------------------------------ #
        # Determine platform-appropriate user-writable base directories       #
        # ------------------------------------------------------------------ #
        _sys = platform.system()

        if _sys == 'Windows':
            # %APPDATA%\newslookout  e.g. C:\Users\<user>\AppData\Roaming\newslookout
            _appdata = os.environ.get('APPDATA', os.path.expanduser('~'))
            _user_base = os.path.join(_appdata, 'newslookout')
            _log_base  = _user_base          # logs beside data on Windows
        elif _sys == 'Darwin':
            # ~/Library/Application Support/newslookout   (macOS convention)
            _user_base = os.path.join(os.path.expanduser('~'),
                                      'Library', 'Application Support', 'newslookout')
            _log_base  = os.path.join(os.path.expanduser('~'),
                                      'Library', 'Logs', 'newslookout')
        else:
            # Linux / BSD: XDG conventions
            _xdg_data  = os.environ.get('XDG_DATA_HOME',
                                        os.path.join(os.path.expanduser('~'), '.local', 'share'))
            _xdg_log   = os.environ.get('XDG_STATE_HOME',
                                        os.path.join(os.path.expanduser('~'), '.local', 'state'))
            _user_base = os.path.join(_xdg_data, 'newslookout')
            _log_base  = os.path.join(_xdg_log,  'newslookout')

        # Config directory follows XDG_CONFIG_HOME on Linux, APPDATA on Windows
        if _sys == 'Windows':
            _config_base = _user_base
        elif _sys == 'Darwin':
            _config_base = os.path.join(os.path.expanduser('~'),
                                        'Library', 'Preferences', 'newslookout')
        else:
            _xdg_cfg = os.environ.get('XDG_CONFIG_HOME',
                                      os.path.join(os.path.expanduser('~'), '.config'))
            _config_base = os.path.join(_xdg_cfg, 'newslookout')

        # ------------------------------------------------------------------ #
        # Assign defaults using the computed base directories                 #
        # ------------------------------------------------------------------ #

        # install prefix is the package directory itself (for bundled plugins)
        self.install_prefix = os.path.dirname(os.path.realpath(__file__))

        self.config_file        = os.path.join(_config_base, 'newslookout.conf')
        self.logfile            = os.path.join(_log_base,    'newslookout.log')
        self.pid_file           = os.path.join(_log_base,    'newslookout.pid')
        self.data_dir           = os.path.join(_user_base,   'data')
        self.archive_base_path  = os.path.join(_user_base,   'data', 'archive')
        self.master_data_dir    = os.path.join(_user_base,   'data', 'master_data')
        self.completed_urls_datafile = os.path.join(_user_base, 'data', 'completed_urls.db')

        # Plugins ship inside the package; contrib lives next to user data
        self.plugins_dir           = os.path.join(self.install_prefix, 'plugins')
        self.plugins_contributed_dir = os.path.join(_user_base, 'plugins_contrib')
