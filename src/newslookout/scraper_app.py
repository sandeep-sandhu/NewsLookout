...
Example usage:
    ...
    app.run(run_date='2025-01-21', max_runtime=3600)
...
from newslookout.config import ConfigManager
...
class NewsLookoutApp:
    ...
    @staticmethod
    def install_default_config(target_config_path: str) -> bool:
        """
        Copy the bundled default configuration template to *target_config_path*
        if the file does not already exist.  Creates parent directories as needed.

        :param target_config_path: Absolute path where the config should be written.
        :return: True if a new config was written, False if it already existed.
        """
        import shutil
        if os.path.isfile(target_config_path):
            return False

        # The template ships inside the package at newslookout/conf/newslookout.conf.template
        template_path = os.path.join(os.path.dirname(__file__), 'conf', 'newslookout.conf.template')
        if not os.path.isfile(template_path):
            logging.warning(
                "Default config template not found at %s – skipping auto-install.", template_path
            )
            return False

        os.makedirs(os.path.dirname(target_config_path), exist_ok=True)
        shutil.copy2(template_path, target_config_path)
        logging.info("Installed default configuration to: %s", target_config_path)
        return True
...
def scrape(config_file: str, run_date: Optional[str] = None,
    ...
    """Convenience function to run a scraping job. Also the main() function or CLI entry point.
    ...
    Example:
        >>> from newslookout.scraper_app import scrape
        >>> stats = scrape('config.conf', run_date='2026-01-13')
        >>> print(f"Processed {stats['urls_processed']} URLs")
    """

    # Auto-install default config if none exists at the expected location
    _tmp_cfg = ConfigManager.__new__(ConfigManager)
    _tmp_cfg.set_default_values()          # compute platform paths without reading a file
    default_cfg_path = _tmp_cfg.config_file
    if not os.path.isfile(config_file) and not os.path.isfile(default_cfg_path):
        NewsLookoutApp.install_default_config(default_cfg_path)
        print(f"A default configuration has been created at:\n  {default_cfg_path}\n"
              f"Please review and customise it before running the application again.")
        sys.exit(0)
    if not os.path.isfile(config_file):
...
def main():
    """CLI entry point for the newslookout console script."""
    import argparse
    import sys

    parser = argparse.ArgumentParser(
        prog='newslookout',
        description='NewsLookout web scraping application for financial news.'
    )
    parser.add_argument(
        '-c', '--config',
        default=None,
        help='Path to configuration file (default: platform config directory)'
    )
    parser.add_argument(
        '-d', '--date',
        default=None,
    ...
    # Resolve config path: use argument, or fall back to platform default
    from newslookout.config import ConfigManager
    config_file = args.config
    if not config_file:
        tmp = ConfigManager.__new__(ConfigManager)
        tmp.set_default_values()
        config_file = tmp.config_file

    # Auto-install default config on first run
    if not os.path.isfile(config_file):
        installed = NewsLookoutApp.install_default_config(config_file)
        if installed:
            print(f"\nNo configuration file found. A default has been created at:\n"
                  f"  {config_file}\n\n"
                  f"Please review and customise it, then run newslookout again.\n")
            sys.exit(0)
        else:
            print(f"ERROR: Configuration file not found: {config_file}", file=sys.stderr)
            sys.exit(1)

    app = NewsLookoutApp(config_file=config_file, run_date=args.date)
    stats = app.run()
    print(f"Done. URLs processed: {stats.get('urls_processed', 0)}")
