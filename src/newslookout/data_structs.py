            ...
            try:
                ...
                if pluginName in previousState and currentState[pluginName] != previousState[pluginName]:
                    ...
                    def _decode_state(val) -> str:
                        """Safely convert an int plugin-state constant to its name."""
                        if isinstance(val, int):
                            decoded = PluginTypes.decodeNameFromIntVal(val)
                            return decoded if decoded is not None else str(val)
                        return str(val)   # already a string

                    statusMessages.append(
                        str(pluginName) +
                        ' changed state to -> ' +
                        _decode_state(currentState[pluginName])
                    )
